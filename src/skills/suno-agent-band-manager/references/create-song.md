...
3. If the song was developed from a WIP fragment file (`docs/wip-*.md`), **mark the WIP file COMPLETED** — do NOT delete it. The fragments are historical record of the brainstorming that led to the song and should be preserved, but the file must not appear as active work on future sessions. To find the WIP that needs marking, run `uv run scripts/scan-wip-status.py "{project-root}" --format json` — its `correlation_warning` flags any active WIP whose working title matches the song you just published. Load `references/reconcile.md` → "The COMPLETED WIP convention" for the exact marker format and the rationale, then apply the marker before ending the session — without it, the next session (especially on a different machine after a portable sync) will incorrectly treat the finished song as pending work. The script finds the candidate; you confirm it's the right WIP and write the marker.
