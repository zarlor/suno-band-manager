...
- `section-map.py` -- Lines a render up with the lyrics it was generated from. Demucs isolates the vocal stem, faster-whisper transcribes it with word timestamps, and the words are aligned to the lyric lines, so each tagged section gets a real start and end even where Suno runs sections together. Per section: mix loudness and the step from the section before, vocal-minus-band, tempo and feel (Beat This! when the PyTorch audio tools are on), and key. It also lists lyric lines not heard and vocals outside the lyric sections, and gives untagged lead-ins and tails their own rows. For consistency it averages instead of switching features off: seeded Demucs shifts, and a consensus over up to 3 seeded Whisper passes. Lines only some passes miss are listed as uncertain. `--lyrics` takes a plain lyrics file or a package/songbook doc. About a minute per song (Whisper runs on CPU); the first run downloads the Whisper medium model (~1.5 GB).
