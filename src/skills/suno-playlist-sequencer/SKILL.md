...
- **Felt BPM governs, not librosa raw.** librosa misreads halftime/double-time routinely (speed metal reads half, doom reads double). Verify felt BPM by ear before claiming tempo continuity across tracks. When the PyTorch audio tools are turned on (`/suno-setup`), the scripts read tempo with Beat This! (the Feedback Elicitor's `beat-grid.py`), which catches most slow-song double-reads, and `batch-full-analysis.py` lists each track's half-time and double-time **feel sections**; otherwise they use librosa. The report says which (`tempo_source`), and the ear still decides.
...
**Canonical input:** the per-band playlist YAML at `docs/{band-slug}-playlist.yaml` — the single source of truth for the band's track sequence. If a band has songbook entries but no playlist YAML, scaffold one via `uv run src/skills/suno-band-profile-manager/scripts/scaffold-playlist.py {band-slug} --from-songbook`, then have the user fill in audio filenames. Schema and lifecycle rules: `suno-band-profile-manager/references/profile-schema.md` "Per-Band Playlist YAML".
...
This produces per-track BPM, overall/entry/exit keys + Camelot codes, energy level, intro/outro energy %, entry/exit tempo (first and last 30 s; from Beat This! when the PyTorch audio tools are on, else librosa), loudness (integrated LUFS, LRA, by thirds, and entry/exit over the first and last 15 s), and per-transition quality (exit-Camelot of N → entry-Camelot of N+1, exit→entry BPM change, and the exit→entry loudness step). Output auto-archives to `docs/audio-analysis/playlists/{band-slug}.json` and refreshes the companion `docs/{band-slug}-playlist-sequencing.md` (AUTOGEN markers preserve hand-curated content). If librosa deps are missing the script returns JSON with install instructions (exit code 2) — surface that and continue with any data already on disk.
