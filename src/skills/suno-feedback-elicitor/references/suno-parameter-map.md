...
| "The tempo changes didn't happen" | One v6 generation keeps one tempo grid, and inline BPM tags don't move it (VENDOR + LOCAL-OBSERVED) | Rewrite the contrast as feel over one pulse (half-time/double-time, sparse vs dense), naming each section's scene in identical words in the style map and the lyric cue. Add BPM + feel words in the style field as soft reinforcement. For a true change: Replace Section on that span with a plain-language prompt, Extend from the boundary with a new style, or splice |
